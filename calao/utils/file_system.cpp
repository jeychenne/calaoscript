...
#include <calao/utils/file_system.hpp>
...
#if CALAO_WINDOWS
	#define CALAO_PATH_SEPARATOR "\\"
	#define CALAO_PATH_SEPARATOR_CHAR '\\'
	#define CALAO_PATH_ENV_DELIMITER ";"
	#define CALAO_USER_DIRECTORY "USERPROFILE"
#else
	#define CALAO_PATH_SEPARATOR "/"
	#define CALAO_PATH_SEPARATOR_CHAR '/'
	#define CALAO_PATH_ENV_DELIMITER ":"
	#define CALAO_USER_DIRECTORY "HOME"
#endif
...
{
	if (!path.ends_with(CALAO_PATH_SEPARATOR_CHAR))
	{
		path.push_back(CALAO_PATH_SEPARATOR_CHAR);
	}
}
...
String full_path(const String &relative_path)
{
	if (relative_path.starts_with('~'))
	{
		...
		String user_dir(getenv(CALAO_USER_DIRECTORY));
...
#elif defined(CALAO_MACOS) || defined(_DEFAULT_SOURCE) || defined(_BSD_SOURCE)
...
#else
	char *dir = getenv(CALAO_USER_DIRECTORY);
...
String separator()
{
	static String sep(CALAO_PATH_SEPARATOR);
	return sep;
}
...
{
	auto pos = path.rfind(CALAO_PATH_SEPARATOR_CHAR);
...
String directory_name(const String &path)
{
	...
	for (i = size; i-- > 0;)
	{
		if (str[i] == CALAO_PATH_SEPARATOR_CHAR)
		{
